...
if uploaded_file is not None:
    ...
    def generate_scatter_plots(data):
        ...
        ax.scatter(data['时间(s)'], data['颈部角度(°)'], label=('颈部角度(°)', fontproperties=simhei_font), alpha=0.7)
        ax.scatter(data['时间(s)'], data['肩部旋转角度(°)'], label=('肩部旋转角度(°)', fontproperties=simhei_font), alpha=0.7)
        ax.set_xlabel('时间(s)', fontproperties=simhei_font)
